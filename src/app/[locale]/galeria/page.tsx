...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    return {
        title: locale === "en" ? "Gallery | Kozłowski Reservoir" : "Galeria | Zalew Kozłowski",
        description: locale === "en"
            ? "View photos from our guests and beautiful catches at the Kozłowski Reservoir. Join our community!"
            : "Zobacz zdjęcia naszych gości i piękne okazy złowione na Zalewie Kozłowskim. Dołącz do naszej społeczności!",
        openGraph: {
            title: locale === "en" ? "Gallery — Kozłowski Reservoir" : "Galeria — Zalew Kozłowski",
            description: locale === "en"
                ? "Beautiful specimens caught at our fishery. See photos from guests!"
                : "Piękne okazy złowione na naszym łowisku. Zobacz zdjęcia gości!",
            url: "/galeria",
        },
    };
}
...
const getFallbackImages = (locale: string): GalleryImage[] => [
    {
        src: "https://images.unsplash.com/photo-1544551763-46a8723ba3f9?q=80&w=2670",
        title: locale === "en" ? "Adam, Carp 12kg" : "Adam, Karp 12kg",
...
export default async function GalleryPage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    let images = getFallbackImages(locale);
        ...
        if (response.items.length > 0) {
            ...
            const mappedImages = response.items.map((item, index) => {
                ...
                return {
                    ...
                    title: (item.fields.title as string) || t("untitled"),
